...
// The identity card itself must not render — including its now-empty <h1> —
// when a profile has nothing to put in it: no portrait, no name, no role, no
// short line, no meta rows, and no CTA (porting convention rule 5).
$abio_t8_show_cta      = $d['site']['contactUrl'] && $d['pitch']['cta'];
$abio_t8_show_identity = $a['portrait'] || $a['name'] || $a['role'] || $a['short'] || $abio_t8_show_meta || $abio_t8_show_cta;

// The "About" card must not render with only its heading when there is
// neither a bio nor gallery items to show (porting convention rule 5).
$abio_t8_show_about = $a['bio'] || ! empty( $d['gallery']['items'] );
?>
<div class="abio-t8">
